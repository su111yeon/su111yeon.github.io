...
/* Tab + swiper 초기화 */
document.addEventListener("DOMContentLoaded", function () {
  const tabContWraps = document.querySelectorAll(".tabContWrap");

  tabContWraps.forEach((wrap) => {
    const tabMenuItems = wrap.querySelectorAll(".tabmenuList li");
    const tabContents = wrap.querySelectorAll(".tabBoxWrap .tabcnt");

    tabMenuItems.forEach((menuItem, index) => {
      menuItem.addEventListener("click", function (e) {
        e.preventDefault();

        // 클릭된 li가 "no-active" 클래스를 가지고 있다면 실행 중단
        ...
        // 모든 tabcnt에서 tabOpen 제거 + 숨기기
        tabContents.forEach((content) => {
          content.classList.remove("tabOpen");
          content.style.display = "none";
        });

        // 현재 탭 콘텐츠 보이기 + tabOpen 추가
        if (currentTabContent) {
          currentTabContent.classList.add("tabOpen");
